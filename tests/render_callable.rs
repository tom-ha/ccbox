...
use ccbox::{
    ansi::strip_ansi, render, width::visible_width, Env, SessionInfo,
};
...
fn scratch() -> Scratch {
    ...
    Scratch { _claude: claude, _home: home, env }
}
...
fn subscription_session_hides_cost_glyph_and_amount() {
    ...
    assert!(!plain.contains('$'), "subscription render should not contain '$':\n{plain}");
}
...
fn ccbox_show_cost_true_restores_cost_on_subscription() {
    ...
    let s = render(&fixture(), &sc.env, 130);
    let plain = strip_ansi(&s);
    assert!(plain.contains('$'), "CCBOX_SHOW_COST=1 must re-enable cost:\n{plain}");
}
...
fn non_subscription_session_shows_cost() {
    ...
    assert!(plain.contains('$'), "API-style session must show cost:\n{plain}");
}
...
fn ccbox_show_cost_false_hides_cost_on_non_subscription() {
    ...
    let out = render(&s, &sc.env, 130);
    let plain = strip_ansi(&out);
    assert!(!plain.contains('$'), "CCBOX_SHOW_COST=0 must hide cost:\n{plain}");
}
...
fn marker_alone_hides_cost_on_fresh_session() {
    ...
    let out = render(&s, &sc.env, 130);
    let plain = strip_ansi(&out);
    assert!(!plain.contains('$'), "marker should hide cost on fresh session:\n{plain}");
}
...
fn burndown_helper_is_removed_from_top_row() {
    ...
    let s = render(&fixture(), &sc.env, 130);
    let plain = strip_ansi(&s);
    assert!(!plain.contains("T-"), "burndown helper must be gone: {plain}");
    for (i, line) in s.lines().enumerate() {
        assert_eq!(visible_width(line), 130, "line {i}: {:?}", strip_ansi(line));
    }
}
...
fn tokens_row_has_augmented_labels() {
    ...
    let s = render(&fixture(), &sc.env, 130);
    let plain = strip_ansi(&s);
    assert!(plain.contains(" in "), "expected augmented 'in' label: {plain}");
    assert!(plain.contains(" out "), "expected augmented 'out' label: {plain}");
}
...
fn venv_renders_when_env_set() {
    ...
    let s = render(&fixture(), &sc.env, 130);
    let plain = strip_ansi(&s);
    assert!(plain.contains("py311"), "venv name should appear in top row:\n{plain}");
    assert!(plain.contains("venv:"), "venv label should appear:\n{plain}");
    for (i, line) in s.lines().enumerate() {
        assert_eq!(visible_width(line), 130, "line {i}: {:?}", strip_ansi(line));
    }
}
...
fn session_id_chip_appears_in_top_border() {
    ...
    assert!(first_plain.contains("abcdef01-23456789"), "expected full session id on top border: {first_plain}");
    // No brackets, no ellipsis.
    assert!(!first_plain.contains('['), "expected no brackets: {first_plain}");
    assert!(!first_plain.contains('…'), "expected no ellipsis: {first_plain}");
...
fn wide_top_row_shows_full_path_branch_and_model_no_vsep_or_burndown() {
    ...
    assert!(content_row.contains("my-project"), "expected path tail: {content_row}");
    assert!(content_row.contains("Sonnet 4.6") || content_row.contains("Sonnet") || content_row.contains("Opus") || content_row.contains("Haiku"), "expected model identity: {content_row}");
    ...
    let inner: String = chars.iter().skip(1).take(chars.len().saturating_sub(2)).collect();
    assert!(!inner.contains('│'), "top row must not have inner vsep glyph: {inner}");
    // Whole output should have no T-… countdown.
    let full = strip_ansi(&out);
    assert!(!full.contains("T-"), "no burndown countdown anywhere: {full}");
}
...
fn populated_rate_limits_writes_marker() {
    ...
    assert!(marker.exists(), "render with populated rate_limits should touch marker");
}
