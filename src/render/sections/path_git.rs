...
impl Renderer {
    ...
    /// Render the git branch chip: `<glyph> <branch>` (clean) or
    /// `<glyph> <branch> ●` (dirty). Uses a file-tree glyph when the repo is
    /// a worktree (or submodule) instead of the branch glyph, so it's
    /// visually distinct. Returns `("", 0)` when there is no branch.
    pub fn branch_chip(&self, git: &GitInfo) -> (String, usize) {
        ...
        let glyph = if git.worktree { GLYPH_GIT_WORKTREE } else { GLYPH_GIT_BRANCH };
        let text = if dirty {
            format!(
                "{}{glyph}{RESET} {}{}{RESET} {}●{RESET}",
                t.branch, t.branch, git.branch, t.alert,
            )
        } else {
            format!(
                "{}{glyph}{RESET} {}{}{RESET}",
...
mod tests {
    ...
    fn git(branch: &str) -> GitInfo {
        GitInfo { branch: branch.into(), ..Default::default() }
    }
    ...
    fn fit_path_head_ellipsis_preserves_tail() {
        let p = "/Users/tomh/git/public/yet-another-statusline-in-rust";
        let r = fit_path_head_ellipsis(p, 30);
        assert!(r.starts_with('…'), "{r}");
        assert_eq!(visible_width(&r), 30);
        // Tail of the input should appear in the output.
        let tail: String = p.chars().rev().take(20).collect::<String>().chars().rev().collect();
    ...
    fn branch_chip_clean_renders_glyph_and_branch() {
        ...
        assert!(plain.contains(GLYPH_GIT_BRANCH), "missing branch glyph: {plain}");
    ...
    fn branch_chip_worktree_swaps_glyph() {
        ...
        let (text, _w) = r.branch_chip(&g);
        let plain = crate::ansi::strip_ansi(&text);
        assert!(plain.contains(GLYPH_GIT_WORKTREE), "expected worktree glyph: {plain}");
        assert!(!plain.contains(GLYPH_GIT_BRANCH), "should not have branch glyph: {plain}");
