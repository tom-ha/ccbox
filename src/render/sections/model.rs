...
impl Renderer {
    ...
    fn model_anchor_pair(&self, model_name: &str) -> (Rgb, Rgb) {
        ...
        let shift = if self.bg_shift == BgShift::Warm { mc.warm_shift } else { mc.cool_shift };
        (mc.anchor, shift)
    }
    ...
    pub fn model_right_section(
        ...
        let glyph = if fast_mode { GLYPH_BURN_FAST } else { GLYPH_THINKING };

        let right_text = if pct > 0 {
            let (anchor, shift) = self.model_anchor_pair(model_name);
            let mut cells: Vec<Cell> = Vec::new();
            cells.push(Cell { ch: GLYPH_MODEL.into(), fg: None, bold: false, italic: false });
            cells.push(Cell { ch: " ".into(), fg: None, bold: false, italic: false });
            cells.push(Cell { ch: " ".into(), fg: None, bold: false, italic: false });
            for ch in model_name.chars() {
                cells.push(Cell { ch: ch.to_string(), fg: None, bold: false, italic: false });
            }
            cells.push(Cell { ch: " ".into(), fg: None, bold: false, italic: false });
            cells.push(Cell { ch: glyph.into(), fg: None, bold: true, italic: false });
            cells.push(Cell { ch: " ".into(), fg: None, bold: true, italic: false });
            cells.push(Cell { ch: " ".into(), fg: None, bold: true, italic: false });
            for ch in model_thinking.chars() {
                cells.push(Cell { ch: ch.to_string(), fg: None, bold: false, italic: true });
            }
            cells.push(Cell { ch: " ".into(), fg: None, bold: false, italic: false });
            let n = cells.len() as i32;
            let pill_l = format!("{}{PILL_LEFT}", pill_gradient_fg(0, 0, n, anchor, shift, pct));
            let pill_r = format!("{}{PILL_RIGHT}", pill_gradient_fg(n, 0, n, anchor, shift, pct));
            let painted = paint_bg_span(&cells, anchor, shift, pct, self.theme.pill_fg_dark, Some(self.theme.pill_fg_light));
...
mod tests {
    ...
    fn model_colour_matches_kind() {
        ...
        assert_eq!(r.model_colour("claude-opus-4-7"), r.theme.model_colors(crate::render::palette::ModelKind::Opus).label);
        assert_eq!(r.model_colour("claude-sonnet-4-6"), r.theme.model_colors(crate::render::palette::ModelKind::Sonnet).label);
    }
