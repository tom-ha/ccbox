...
impl Renderer {
    ...
    pub fn spec_rgb_at(&self, t: f64, idx: usize, three_stops: bool) -> Rgb {
        ...
        let c0 = stops[s0];
        let c1 = stops[s1];
        let lerp = |a: u8, b: u8| -> i32 { (a as f64 + (b as f64 - a as f64) * u).trunc() as i32 };
        (
            lerp(c0.0, c1.0).clamp(0, 255) as u8,
            lerp(c0.1, c1.1).clamp(0, 255) as u8,
            lerp(c0.2, c1.2).clamp(0, 255) as u8,
        )
    }
    ...
    pub fn openspec_bar(
        ...
        let bar_empty = format!(
            "{}{}\x1b[0m",
            self.theme.spec_empty_ansi,
            bar::HEAVY.repeat(empty as usize)
        );
