...
impl TokensCostExtras {
    pub fn new() -> Self {
        Self { show_cost: true, ..Default::default() }
    }
}
...
fn cost_cluster_width(sess: f64, day: f64) -> i32 {
    ...
    (1 + c1.chars().count() + " sess · ".chars().count() + c2.chars().count() + " today".chars().count()) as i32
}
...
pub fn sparkline_bar_w(
    ...
    let cost_w = if show_cost { cost_cluster_width(sess_cost, day_cost) } else { 0 };
...
impl Renderer {
    pub fn tokens_cost(
        ...
        let t = self.theme;
        let in_icon = if extras.in_active { "\x1b[1m↓\x1b[0m" } else { "↓" };
        let out_icon = if extras.out_active { "\x1b[1m↑\x1b[0m" } else { "↑" };
        ...
        let rate_label = format!(
            ...
            t.tok_icon, t.tok, fmt_tok(tok_rate), t.label,
        );
        ...
        let sparkline = if bar_w <= 0 {
            ...
            let (_top, bot) = self.gradient().sparkline(&history, !extras.spark_history.is_empty());
            bot
        };
...
mod tests {
    ...
    fn tokens_cost_hidden_cost_blanks_cluster() {
        let r = Renderer::default();
        let extras = TokensCostExtras { show_cost: false, ..Default::default() };
        let result = r.tokens_cost(1, 2, 0.42, 12.34, 0, &extras, 160);
        let plain = strip_ansi(&result.line);
        assert!(!plain.contains('$'), "no dollar sign when cost hidden: {plain}");
    ...
    fn tokens_cost_sparkline_present_with_history() {
        ...
        assert!(
            plain.chars().any(|c| ('\u{1FB00}'..='\u{1FBFF}').contains(&c)),
        ...
        for &show_cost in &[true, false] {
            for &box_width in &[80i32, 100, 130, 160, 200] {
                for &tok_rate in &[0u64, 12, 4567, 999_999] {
                    ...
                    let extras = TokensCostExtras { show_cost, ..TokensCostExtras::new() };
