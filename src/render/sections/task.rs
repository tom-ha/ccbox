...
impl Renderer {
    ...
    pub fn task_row(&self, tasks: &TaskList, width: i32, _step: usize) -> String {
        ...
        let raw_subject = active.map(|task| {
    ...
    pub fn task_board(&self, tasks: &TaskList, width: i32) -> Vec<String> {
        ...
        let todo: Vec<_> = tasks.tasks.iter().filter(|t| t.status == TaskStatus::Pending).collect();
        let doing: Vec<_> = tasks.tasks.iter().filter(|t| t.status == TaskStatus::InProgress).collect();
        let done: Vec<_> = tasks.tasks.iter().filter(|t| t.status == TaskStatus::Completed).collect();

        let header = |label: &str, count: usize, w: i32| -> String {
            let raw = format!("{label} {count}");
            let truncated = truncate_ellipsis(&raw, w as usize);
            let pad = (w - visible_width(&truncated) as i32).max(0) as usize;
            format!("{}{BOLD}{truncated}{RESET}{}", t.label, " ".repeat(pad))
        };

        let cell_for = |i: usize, list: &[&crate::data::task_list::Task], col_w: i32, bullet: &str, bullet_clr: &str, text_clr: &str, overflow: usize| -> String {
            // Last visible row shows "+N more" when overflow > 0.
            let is_last = i + 1 == MAX_BOARD_ROWS.min(list.len() + if overflow > 0 { 1 } else { 0 });
            if overflow > 0 && is_last {
                let raw = format!("+{overflow} more");
                let truncated = truncate_ellipsis(&raw, col_w as usize);
                let pad = (col_w - visible_width(&truncated) as i32).max(0) as usize;
            ...
            let task = list[i];
            let subj_src = if task.active_form.is_empty() { &task.subject } else { &task.active_form };
            ...
            let pad = (col_w - visible_width(&cell_raw) as i32).max(0) as usize;
            format!("{bullet_clr}{bullet}{RESET} {text_clr}{subj}{RESET}{}", " ".repeat(pad))
        };
        ...
        let row_count = [todo.len(), doing.len(), done.len()]
            .iter().copied().max().unwrap_or(0).min(MAX_BOARD_ROWS);
        ...
        for i in 0..row_count {
            ...
            let c2 = cell_for(i, &doing, col2_w, BULLET_DOING, t.tok, t.ctx, doing_overflow);
            let c3 = cell_for(i, &done, col3_w, BULLET_DONE, t.dim_green, t.ctx_dim, done_overflow);
...
mod tests {
    ...
    fn make_af(status: TaskStatus, subject: &str, af: &str) -> Task {
        Task { id: 1, subject: subject.into(), active_form: af.into(), status }
    }
    fn list(tasks: Vec<Task>) -> TaskList {
        TaskList { tasks, last_event_ts: 0.0 }
    }
    ...
    fn task_row_inline_includes_active_subject() {
        let r = Renderer::default();
        let l = list(vec![
            make_af(TaskStatus::InProgress, "ship-the-feature", "shipping the feature"),
        ]);
        let row = r.task_row(&l, 140, 0);
        let plain = strip_ansi(&row);
    ...
    fn task_row_truncates_long_subject() {
        ...
        let plain = strip_ansi(&row);
        assert!(plain.contains('…'), "expected ellipsis when subject too long: {plain}");
    }
    ...
    fn task_row_narrow_drops_subject() {
        let r = Renderer::default();
        let l = list(vec![
            make_af(TaskStatus::InProgress, "alpha", "alpha"),
        ]);
        let row = r.task_row(&l, 50, 0);
        let plain = strip_ansi(&row);
        assert!(!plain.contains("alpha"), "narrow row should not include subject: {plain}");
    }
    ...
    fn task_board_returns_multi_row_at_wide_width() {
        ...
        assert!(rows.len() >= 3, "expected header + content rows: {}", rows.len());
    ...
    fn task_board_narrow_degrades_to_inline() {
        let r = Renderer::default();
        let l = list(vec![
            make(TaskStatus::InProgress, "ship-the-feature"),
        ]);
        let rows = r.task_board(&l, 80);
        assert_eq!(rows.len(), 1, "narrow should auto-degrade to inline: {rows:?}");
    }
    ...
    fn task_board_empty_columns_render_blank_cells() {
        let r = Renderer::default();
        let l = list(vec![
            make(TaskStatus::InProgress, "ship"),
        ]);
