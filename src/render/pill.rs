...
impl Default for Pill {
    fn default() -> Self {
        Self { start: -1, end: -1, anchor: (0, 0, 0), shift: (0, 0, 0), pct: 0 }
    }
}

impl Pill {
    pub fn active(&self) -> bool { self.pct > 0 }
    ...
    pub fn border_char(&self, col: i32, edge: Edge) -> &'static str {
        ...
        match edge {
            Edge::Top => {
                if col == self.start { glyphs::PILL_TL }
                else if col == self.end { glyphs::PILL_TR }
                else { glyphs::PILL_TOP }
            }
            Edge::Bottom => {
                if col == self.start { glyphs::PILL_BL }
                else if col == self.end { glyphs::PILL_BR }
                else { glyphs::PILL_BOT }
            }
        }
    }

    pub fn border_fg(&self, col: i32) -> String {
        pill_gradient_fg(col - self.start, 0, self.end - self.start, self.anchor, self.shift, self.pct)
    }
}
...
pub fn pill_gradient_fg(col: i32, pill_start: i32, pill_end: i32, anchor: Rgb, shift: Rgb, pct: i32) -> String {
...
pub fn paint_bg_span(cells: &[Cell], anchor: Rgb, shift: Rgb, pct: i32, pill_fg_dark: Rgb, pill_fg_light: Option<Rgb>) -> String {
    let c0 = scale(anchor, pct);
    let c1 = scale(shift, pct);
...
mod tests {
    ...
    fn active_pill_borders_at_ends() {
        let p = Pill { start: 5, end: 10, anchor: (255, 0, 0), shift: (0, 0, 255), pct: 100 };
    ...
    fn pill_gradient_fg_starts_and_ends_at_endpoints() {
        ...
        assert_eq!(pill_gradient_fg(0, 0, 10, anchor, shift, 100), "\x1b[38;2;100;0;0m");
        assert_eq!(pill_gradient_fg(10, 0, 10, anchor, shift, 100), "\x1b[38;2;0;100;0m");
    }
    ...
    fn pill_gradient_fg_clamps_outside_range() {
        let anchor = (50, 0, 0);
        let shift  = (0, 0, 50);
        assert_eq!(pill_gradient_fg(-5, 0, 10, anchor, shift, 100), "\x1b[38;2;50;0;0m");
        assert_eq!(pill_gradient_fg(99, 0, 10, anchor, shift, 100), "\x1b[38;2;0;0;50m");
    }

    #[test]
    fn paint_bg_span_emits_background_for_each_distinct_cell() {
        let cells = vec![
            Cell { ch: "a".into(), fg: None, bold: false, italic: false },
            Cell { ch: "b".into(), fg: None, bold: false, italic: false },
            Cell { ch: "c".into(), fg: None, bold: false, italic: false },
        ];
