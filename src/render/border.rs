...
impl<'a> BorderRenderer<'a> {
    pub fn new(theme: &'a Theme) -> Self {
        Self {
            gradient: GradientEngine::new(theme),
            theme,
        }
    }
    ...
    fn dim_for_col(col: i32, elbows: &HashSet<i32>) -> f64 {
        ...
        if d == 0 {
            ...
            (1.0 - (1.0 - DIM_MIN) * (d as f64 / DIM_RAMP)).max(DIM_MIN)
        }
    }
    ...
    pub fn border_separator(&self, width: i32, ups: &[i32], fill: f64, left_chip: &str) -> String {
        let ups_set: HashSet<i32> = ups.iter().copied().collect();
        let mut parts = String::new();
        parts.push_str(&self.gradient.grad_at(0, width, 1.0, fill));
        parts.push('├');
        ...
        let inner_w = (width - 2).max(0);
        let chip_w = visible_width(left_chip) as i32;
        let chip_fits = !left_chip.is_empty() && chip_w > 0 && chip_w <= inner_w;
        let chip_cells = if chip_fits { chip_w } else { 0 };

        if chip_fits {
            parts.push_str(left_chip);
            parts.push_str(RESET);
        }
        for i in chip_cells..(width - 2) {
            let ch = if ups_set.contains(&(i + 2)) {
                '┴'
            } else {
                '─'
    ...
    pub fn border_separator_dim(
        ...
        let inner_w = (width - 2).max(0);
        let chip_w = visible_width(left_chip) as i32;
        // Don't render the chip when a pill is active — they'd visually clash.
        let chip_fits =
            !left_chip.is_empty() && chip_w > 0 && chip_w <= inner_w && !p.active();
        let chip_cells = if chip_fits { chip_w } else { 0 };
        ...
        if p.active() && p.start <= 1 {
            ...
            parts.push_str(&self.gradient.grad_at(
                0,
                width,
                Self::dim_for_col(1, &elbow_cols),
                fill,
            ));
            parts.push('├');
        }
        ...
        for i in chip_cells..(width - 2) {
            ...
            if !pc.is_empty() {
                ...
                parts.push_str(&self.gradient.grad_at(
                    i + 1,
                    width,
                    Self::dim_for_col(col, &elbow_cols),
                    fill,
                ));
                parts.push(ch);
            }
        ...
        if p.active() && p.start <= width && width <= p.end {
            ...
            parts.push_str(&self.gradient.grad_at(
                width - 1,
                width,
                Self::dim_for_col(width, &elbow_cols),
                fill,
            ));
            parts.push('┤');
        }
    ...
    pub fn border_line(
        ...
        if !right_pill.is_empty() {
            ...
            let pad = pad(width - 2, content_w + pill_w);
...
mod tests {
    ...
    fn br() -> BorderRenderer<'static> {
        BorderRenderer::new(&CLAUDE_DARK)
    }
    ...
    #[test]
    fn separator_with_left_chip_anchors_left() {
        let s = br().border_separator(40, &[], 1.0, " OpenSpec ");
        assert_eq!(visible_width(&s), 40);
        let plain = crate::ansi::strip_ansi(&s);
        assert!(
            plain.starts_with("├ OpenSpec "),
            "expected chip flush left: {plain:?}",
        );
        assert!(plain.ends_with('┤'));
    }

    #[test]
    fn separator_dim_with_left_chip_anchors_left() {
        let s = br().border_separator_dim(40, &[], &[], 1.0, None, Edge::Bottom, " OpenSpec ");
        assert_eq!(visible_width(&s), 40);
        let plain = crate::ansi::strip_ansi(&s);
        assert!(
            plain.starts_with("├ OpenSpec "),
    ...
    fn top_border_chip_anchors_to_right_corner() {
        ...
        assert!(
            plain.ends_with("abcde╮"),
            "expected chip flush right, got {plain:?}"
        );
    }
