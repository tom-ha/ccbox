...
impl<'a> GradientEngine<'a> {
    ...
    pub fn default_engine() -> GradientEngine<'static> {
        GradientEngine { theme: DEFAULT_THEME }
    }
    ...
    fn sample(&self, stops: &[(f32, Rgb)], t: f64, dim: f64) -> Rgb {
        ...
        for i in 0..stops.len().saturating_sub(1) {
            ...
            let (t1, c1) = stops[i + 1];
            if t <= t1 as f64 {
                let u = if (t1 - t0) > 0.0 { (t - t0 as f64) / (t1 - t0) as f64 } else { 0.0 };
    ...
    fn spark_flat(idx: i32) -> (&'static str, &'static str) {
        if idx <= 0 { return (" ", SPARK_CHARS[0]); }
        if idx <= 8 { return (" ", SPARK_CHARS[(idx - 1) as usize]); }
        (SPARK_CHARS[(idx - 9) as usize], "█")
    }
    fn spark_rise(idx: i32) -> (&'static str, &'static str) {
        if idx <= 0 { return (" ", SPARK_CHARS[0]); }
        if idx <= 3 { return (" ", glyphs::SPARK_RISE_SMALL); }
        if idx <= 7 { return (" ", glyphs::SPARK_RISE_MED); }
        if idx <= 8 { return (" ", glyphs::SPARK_RISE_TALL); }
        (glyphs::SPARK_RISE_TOP, glyphs::SPARK_RISE_TALL)
    }
    fn spark_fall(idx: i32) -> (&'static str, &'static str) {
        if idx <= 0 { return (" ", SPARK_CHARS[0]); }
        if idx <= 3 { return (" ", glyphs::SPARK_FALL_SMALL); }
        if idx <= 7 { return (" ", glyphs::SPARK_FALL_MED); }
        if idx <= 8 { return (" ", glyphs::SPARK_FALL_TALL); }
    ...
    pub fn sparkline(&self, history: &[i32], live: bool) -> (String, String) {
        ...
        let indices: Vec<i32> = history
            .iter()
            .map(|&v| {
                let r = if max_val > 0 { v as f64 / max_val as f64 } else { 0.0 };
        ...
        for (i, &idx) in indices.iter().enumerate() {
            ...
            let (bot_clr, top_clr) = if live && i == last_i {
                (self.spark_color(ratio_bot, LIVE_DIM as f64), self.spark_color(ratio_top, LIVE_DIM as f64))
            } else {
                (self.spark_color(ratio_bot, 1.0), self.spark_color(ratio_top, 1.0))
            };
...
mod tests {
    ...
    #[test] fn spark_color_dim_one_matches_default() {
        let e = eng();
        assert_eq!(e.spark_color(0.5, 1.0), e.spark_color(0.5, 1.0));
    }

    #[test] fn sparkline_empty_history() {
        let (t, b) = eng().sparkline(&[], false);
        assert!(t.is_empty() && b.is_empty());
    }

    #[test] fn sparkline_history_length_matches_cells() {
    ...
    #[test] fn gradient_bar_filled_width_visible() {
        use crate::width::visible_width;
