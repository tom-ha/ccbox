...
pub const OPUS: Rates = Rates { input: 15.00, output: 75.00 };
pub const HAIKU: Rates = Rates { input: 0.80, output: 4.00 };
pub const DEFAULT_RATES: Rates = Rates { input: 3.00, output: 15.00 };
...
mod tests {
    ...
    #[test] fn opus_rates() {
        ...
        assert_eq!(r.input, 15.00); assert_eq!(r.output, 75.00);
    }
    #[test] fn haiku_rates_via_id() {
        let r = rates_for("claude-haiku-4-5-20251001");
        assert_eq!(r.input, 0.80); assert_eq!(r.output, 4.00);
    }
    #[test] fn sonnet_default_rates() {
        let r = rates_for("claude-sonnet-4-6");
        assert_eq!(r.input, 3.00); assert_eq!(r.output, 15.00);
    }
    #[test] fn unknown_model_default_rates() {
        let r = rates_for("gpt-5");
        assert_eq!(r.input, 3.00); assert_eq!(r.output, 15.00);
    }
    #[test] fn case_insensitive_opus() {
        let r = rates_for("CLAUDE-OPUS-4");
        assert_eq!(r.input, 15.00);
    }
    #[test] fn case_insensitive_haiku() {
        let r = rates_for("HAIKU 4.5");
        assert_eq!(r.input, 0.80);
    }

    // --- session_cost -------------------------------------------------------
    #[test] fn session_cost_sonnet() {
        let r = rates_for("claude-sonnet-4-6");
        let u = Usage { input_tokens: 1_000_000, output_tokens: 1_000_000, ..Default::default() };
        approx(session_cost(r, &u), 18.0);
    }
    #[test] fn session_cost_opus_cache() {
        let r = rates_for("opus");
        let u = Usage { cache_creation_input_tokens: 1_000_000, cache_read_input_tokens: 1_000_000, ..Default::default() };
        approx(session_cost(r, &u), 20.25);
    }
    #[test] fn session_cost_haiku() {
        let r = rates_for("haiku");
        let u = Usage { input_tokens: 2_000_000, output_tokens: 1_000_000, ..Default::default() };
        approx(session_cost(r, &u), 5.60);
    }
    #[test] fn session_cost_default_zero() {
        let r = rates_for("");
        approx(session_cost(r, &Usage::default()), 0.0);
    }

    // --- day_cost -----------------------------------------------------------
    #[test] fn day_cost_via_log() {
        let r = rates_for("claude-sonnet-4-6");
        let log = DayTotals { day_in: 500_000, day_cache_read: 200_000, day_out: 100_000 };
