...
pub fn session_elapsed(transcript_path: &str, now: Option<f64>) -> String {
    ...
    let mtime_secs = mtime.duration_since(UNIX_EPOCH).map(|d| d.as_secs_f64()).unwrap_or(0.0);
...
impl RowSpec {
    pub fn new(kind: RowKind) -> Self { Self { kind, ..Default::default() } }
...
pub fn render_layout(spec: &LayoutSpec, r: &Renderer) -> Vec<String> {
    ...
    for row in &spec.rows {
        match row.kind {
            RowKind::TopBorder => out.push(border.border_top(spec.width, &spec.top_right_chip, &row.downs, spec.fill, row.pill.as_ref())),
            RowKind::BottomBorder => out.push(border.border_bottom(spec.width, &row.ups, spec.fill)),
            RowKind::Separator | RowKind::SeparatorSeam => out.push(border.border_separator(spec.width, &row.ups, spec.fill)),
            RowKind::SeparatorDim => out.push(border.border_separator_dim(spec.width, &row.downs, &row.ups, spec.fill, row.pill.as_ref(), row.pill_edge)),
            RowKind::Content => out.push(border.border_line(&row.content, spec.width, spec.fill, &row.bg_lead, &row.bg_trail, row.pill_flush, &row.right_pill)),
        }
    }
    out
}
...
/// Top-level entry: pick the right layout, build it, return joined lines.
pub fn render(session: &SessionInfo, env: &Env, width: i32, r: &Renderer) -> String {
    ...
    let spec = if width < NARROW_WIDTH as i32 {
        narrow::build_narrow(session, env, width, r)
    } else if width < MEDIUM_WIDTH as i32 {
        medium::build_medium(session, env, width, r)
    } else {
        wide::build_wide(session, env, width, r)
    };
...
mod tests {
    ...
    fn touch_mtime(path: &Path, mtime: SystemTime) {
        ...
        let times = [LibcTimeval { tv_sec: secs, tv_usec: 0 }, LibcTimeval { tv_sec: secs, tv_usec: 0 }];
    ...
    #[repr(C)]
    struct LibcTimeval { tv_sec: i64, tv_usec: i64 }
    extern "C" {
        #[link_name = "utimes"]
        fn libc_utimes(path: *const i8, times: *const LibcTimeval) -> i32;
    }

    #[test] fn empty_path_returns_empty() { assert_eq!(session_elapsed("", None), ""); }
    #[test] fn missing_file_returns_empty() {
        let dir = tempdir().unwrap();
        assert_eq!(session_elapsed(dir.path().join("nope").to_str().unwrap(), None), "");
    }
    #[test] fn five_minutes_old() {
        let dir = tempdir().unwrap();
        ...
        assert_eq!(session_elapsed(path.to_str().unwrap(), Some(2_000_000_000.0)), "5m");
    }
    #[test] fn two_hours_two_minutes_old() {
        let dir = tempdir().unwrap();
        let path = dir.path().join("t.jsonl");
        File::create(&path).unwrap().write_all(b"{}").unwrap();
        ...
        assert_eq!(session_elapsed(path.to_str().unwrap(), Some(now_secs as f64)), "2h2m");
    }

    #[test] fn session_id_chip_empty_when_inputs_empty() {
        assert_eq!(session_id_chip(&crate::theme::builtin::CLAUDE_DARK, "", ""), "");
    }

    #[test] fn session_id_chip_renders_full_id_and_clock_elapsed() {
        let s = session_id_chip(&crate::theme::builtin::CLAUDE_DARK, "51e977df-abc123def", "1h23m");
    ...
    #[test] fn session_id_chip_short_id_renders_verbatim() {
