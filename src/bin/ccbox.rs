...
fn main() -> ExitCode {
    ...
    let out = render_with(&session, &env_struct, width, theme, bg_shift);
    ...
    eprintln!(
        "Usage: ccbox [--theme NAME] [--width COLS] [--full-width] [--bg-shift warm|cool]\n\
         ...
         \n\
           ...
           CCBOX_TASKS           inline (default) | board — render the task row as a single\n\
                                 inline kanban (default) or a multi-line board (board mode\n\
                                 auto-degrades to inline below ~100 columns).\n\
