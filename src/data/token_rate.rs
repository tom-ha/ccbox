...
fn read_lines(log: &Path) -> Vec<(f64, String, u64, u64)> {
    ...
    for ln in contents.lines() {
        ...
        let ts: f64 = match parts[0].parse() { Ok(v) => v, Err(_) => continue };
        let ti: u64 = match parts[2].parse() { Ok(v) => v, Err(_) => continue };
        let to: u64 = match parts[3].parse() { Ok(v) => v, Err(_) => continue };
...
impl TokenRate {
    ...
    pub fn update(claude_dir: &Path, session_id: &str, total_in: u64, total_out: u64, now: f64) -> u64 {
    ...
    pub fn history(claude_dir: &Path, session_id: &str, n_buckets: usize, window: f64, now: f64) -> Vec<i32> {
    ...
    pub fn recently_active(claude_dir: &Path, session_id: &str, now: f64, window: f64) -> (bool, bool) {
        if session_id.is_empty() {
            return (false, false);
        }
        let log = claude_dir.join("statusline-token-rate.log");
...
mod tests {
    ...
    fn history_spreads_delta_across_spanned_buckets() {
        ...
        assert!(nonzero >= 4, "expected delta spread across ~5 buckets, got {h:?}");
        let total: i32 = h.iter().sum();
        assert!((total - 1000).abs() <= 5, "buckets must sum to ~delta, got {total}");
    }
