...
/// Compute the non-negative space remaining in `content_w` after `used` columns
/// have been consumed. Saturates at zero so callers can `" ".repeat(pad(...))`
/// without worrying about `usize` underflow.
pub fn pad(content_w: i32, used: i32) -> usize {
    (content_w - used).max(0) as usize
}
...
mod tests {
    ...
    #[test]
    fn pad_basic() {
        assert_eq!(pad(10, 3), 7);
    }
    #[test]
    fn pad_zero_when_equal() {
        assert_eq!(pad(10, 10), 0);
    }
    #[test]
    fn pad_zero_when_overrun() {
        assert_eq!(pad(10, 15), 0);
    }
    #[test]
    fn pad_zero_content() {
        assert_eq!(pad(0, 5), 0);
    ...
    #[test]
    fn pad_negative_content_saturates() {
        assert_eq!(pad(-3, 0), 0);
    }
